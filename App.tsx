      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-24 bg-white/[0.02] border-b border-white/5 flex items-center justify-between px-10 shrink-0">
          ...
          <div className="flex items-center gap-6">
            <Link to="/" className="text-[10px] font-black text-slate-400 hover:text-brand-500 uppercase tracking-widest transition-colors">Volver a la Landing ↗</Link>
...
const AdminDashboard = () => {
  ...
  const userEmail = session?.user?.email || 'tu-email@dominio.com';
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {[
          ...
          { label: 'Ingresos MRR', val: '4.200€', color: 'purple', icon: '💰' },
          { label: 'Uptime', val: '99.9%', color: 'emerald', icon: '⚡' },
        ].map((s, i) => (
          <div key={i} className="bg-white/5 border border-white/5 p-8 rounded-[2.5rem] relative overflow-hidden group hover:bg-white/10 transition-all hover:border-brand-500/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white/5 border border-white/5 rounded-[2.5rem] p-10">
          ...
          <div className="space-y-6">
             {[1,2,3].map(i => (
               <div key={i} className="flex items-center justify-between p-4 bg-white/[0.02] rounded-2xl border border-white/5 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-4">
                     <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-sm font-bold">{i}</div>
                     <div>
                        <div className="text-sm font-black">Empresa {i} ha actualizado su plan</div>
                        <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Módulo de Suscripciones</div>
                     </div>
                  </div>
                  <div className="text-[9px] font-black text-brand-500 uppercase">Éxito</div>
               </div>
             ))}
          </div>
        </div>
        ...
        <div className="bg-brand-600/10 border border-brand-500/20 rounded-[2.5rem] p-10 flex flex-col justify-between">
           <div>
              <h3 className="text-xs font-black text-brand-500 uppercase tracking-widest mb-4">Ayuda de Administrador</h3>
              <p className="text-sm text-slate-300 leading-relaxed font-medium">
                Si te has registrado con tu email real y no puedes entrar, copia este código en el <b>SQL Editor</b> de Supabase para darte permisos de jefe:
              </p>
              <div className="mt-6 p-4 bg-black/40 rounded-2xl border border-white/5 font-mono text-[10px] text-brand-400 break-all select-all">
                UPDATE profiles SET is_superadmin = true WHERE email = '{userEmail}';
              </div>
           </div>
           <div className="mt-8">
              <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Estado de la cuenta</div>
              <div className="flex items-center gap-2">
                 <div className="px-3 py-1 bg-brand-500 text-white text-[9px] font-black rounded-full uppercase">SuperAdmin Activo</div>
              </div>
           </div>
        </div>
      </div>
...
const AdminTenants = () => {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-2xl font-black text-white tracking-tight">Directorio de Tenants</h3>
      <div className="bg-white/5 border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
        <table className="w-full text-left">
          <thead className="bg-white/5 text-slate-500 text-[10px] font-black uppercase tracking-widest">
            <tr>
              <th className="px-8 py-6">Compañía</th>
              <th className="px-8 py-6">Plan</th>
              <th className="px-8 py-6 text-right">Acceso</th>
            </tr>
          </thead>
            ...
            {tenants.map(t => (
              <tr key={t.id} className="hover:bg-white/[0.02] transition-colors">
                <td className="px-8 py-6">
                   ...
                   <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">/c/{t.slug}</div>
                </td>
                <td className="px-8 py-6">
                   <span className="px-3 py-1 bg-slate-800 text-slate-400 text-[9px] font-black uppercase rounded-full border border-white/5">{t.plan}</span>
                </td>
                <td className="px-8 py-6 text-right">
                   <button onClick={() => window.open(`#/t/${t.slug}/dashboard`, '_blank')} className="text-brand-500 hover:text-brand-400 text-[10px] font-black uppercase tracking-widest underline decoration-2 underline-offset-4 transition-all">Impersonar ↗</button>
                </td>
              </tr>
            ))}
          </tbody>
...
const AdminCMS = () => {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-2xl font-black text-white tracking-tight">Editor de Contenidos Globales</h3>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
           <div className="bg-slate-900 border border-white/10 p-12 rounded-[3rem] w-full max-w-xl shadow-2xl animate-in zoom-in-95 duration-300">
              <h4 className="text-xl font-black text-white mb-8 tracking-tight">Editar Clave: <span className="text-brand-500 uppercase text-sm font-mono">{editing.key}</span></h4>
              <div className="space-y-6">
                 <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Contenido Español (ES)</label>
                    <textarea 
                      value={editing.es} 
                      onChange={e => setEditing({...editing, es: e.target.value})}
                      className="w-full bg-black/40 border border-white/5 rounded-2xl p-4 text-sm focus:ring-2 focus:ring-brand-500 h-32 text-slate-300 outline-none"
                    />
                 </div>
                 <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Contenido Català (CA)</label>
                    <textarea 
                      value={editing.ca} 
              ...
              <div className="flex gap-4 mt-10">
                 <Button onClick={() => setEditing(null)} className="flex-1">Guardar Cambios</Button>
                 <Button variant="ghost" onClick={() => setEditing(null)} className="px-8">Cerrar</Button>
              </div>
           </div>
        </div>
      )}
        ...
        <table className="w-full text-left">
          <thead className="bg-white/5 text-slate-500 text-[10px] font-black uppercase tracking-widest">
            <tr>
              ...
              <th className="px-8 py-6">Valor Actual (ES)</th>
          ...
          <tbody className="divide-y divide-white/5">
            {content.map(item => (
              <tr key={item.key} className="hover:bg-white/[0.02] transition-colors">
                <td className="px-8 py-6 font-mono text-[10px] text-brand-400 font-black uppercase tracking-widest">{item.key}</td>
                <td className="px-8 py-6 text-sm text-slate-300 truncate max-w-md">{item.es}</td>
                <td className="px-8 py-6 text-right">
                  <button onClick={() => setEditing(item)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all">Modificar</button>
                </td>
              </tr>
            ))}
          </tbody>
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    else { alert("Error: Credenciales no válidas. Prueba los botones de demo de abajo."); setLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] p-6">
      <div className="max-w-md w-full animate-in zoom-in-95 duration-500">
        ...
        {/* BOTÓN SUPERADMIN DESTACADO */}
        <div className="mb-8 p-1.5 bg-slate-900 rounded-[2rem] shadow-2xl">
           <button 
             onClick={() => { enterDemoMode(true); navigate('/admin/dashboard'); }}
             className="w-full py-4 bg-gradient-to-r from-brand-600 to-brand-400 hover:from-brand-500 hover:to-brand-300 text-white rounded-[1.8rem] font-black uppercase tracking-[0.2em] text-[11px] transition-all transform hover:-translate-y-1 shadow-xl flex items-center justify-center gap-3"
           >
        ...
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden">
          ...
          <h2 className="text-3xl font-black mb-10 text-gray-900 tracking-tight leading-none">Inicia sesión</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <Button type="submit" variant="dark" className="w-full py-5 text-xs" disabled={loading}>{loading ? 'Accediendo...' : 'ENTRAR AL SISTEMA'}</Button>
          </form>
          ...
          <div className="grid grid-cols-1 gap-3">
            <Button onClick={() => { enterDemoMode(); navigate('/t/demo/dashboard'); }} variant="secondary" className="w-full">
...
const Signup = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <div className="max-w-md w-full bg-white p-12 rounded-[3rem] shadow-2xl border border-gray-100 animate-in slide-in-from-bottom-8 duration-500">
        <div className="flex justify-between items-center mb-10">
            <Link to="/" className="font-black text-brand-600 tracking-tighter text-xl">← ACME</Link>
            <LanguageSwitcher />
        </div>
        <h2 className="text-3xl font-black mb-10 text-gray-900 tracking-tight leading-none">Crea tu cuenta</h2>
        <form onSubmit={handleSignup} className="space-y-6">
          ...
          <Button type="submit" className="w-full py-5 text-xs" variant="dark" disabled={loading}>{loading ? 'Procesando...' : 'REGISTRARME AHORA'}</Button>
        </form>
        <p className="mt-8 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-40 text-center">
        <div className="inline-block px-4 py-1.5 bg-brand-50 text-brand-600 text-[10px] font-black uppercase tracking-widest rounded-full mb-8">Novedad: SaaS Multi-Tenant v1.0</div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 animate-in slide-in-from-bottom-12 duration-1000 delay-200">
           <Link to={session ? dashboardLink : "/signup"} className="w-full sm:w-auto px-12 py-6 bg-slate-900 text-white rounded-[2rem] text-sm font-black uppercase tracking-widest shadow-2xl hover:bg-black transition-all hover:-translate-y-1">{session ? 'IR AL PANEL' : 'EMPEZAR GRATIS'}</Link>
...
const TenantLayout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#fcfcfc] overflow-hidden">
      <aside className="w-80 bg-white border-r border-gray-100 flex flex-col shrink-0 z-30">
        ...
        <nav className="flex-1 p-6 space-y-2 mt-2">
          <Link to={`/t/${slug}/dashboard`} className={`flex items-center gap-4 px-6 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest transition-all ${isActive('dashboard') ? 'bg-brand-600 text-white shadow-xl shadow-brand-500/20' : 'text-gray-400 hover:bg-gray-50'}`}><span>📊</span> {t('dashboard')}</Link>
          <Link to={`/t/${slug}/customers`} className={`flex items-center gap-4 px-6 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest transition-all ${isActive('customers') ? 'bg-brand-600 text-white shadow-xl shadow-brand-500/20' : 'text-gray-400 hover:bg-gray-50'}`}><span>👥</span> {t('customers')}</Link>
          <Link to={`/t/${slug}/quotes`} className={`flex items-center gap-4 px-6 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest transition-all ${isActive('quotes') ? 'bg-brand-600 text-white shadow-xl shadow-brand-500/20' : 'text-gray-400 hover:bg-gray-50'}`}><span>📄</span> {t('quotes')}</Link>
          <Link to={`/t/${slug}/settings`} className={`flex items-center gap-4 px-6 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest transition-all ${isActive('settings') ? 'bg-brand-600 text-white shadow-xl shadow-brand-500/20' : 'text-gray-400 hover:bg-gray-50'}`}><span>⚙️</span> {t('settings')}</Link>
        </nav>
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {[
            { l: t('total_revenue'), v: '14.280 €', i: '💰', c: 'blue' },
            { l: t('active_quotes'), v: '12', i: '⏳', c: 'orange' },
            { l: t('total_customers'), v: '48', i: '👥', c: 'brand' },
          ].map((s, i) => (
            <div key={i} className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-50 hover:shadow-2xl transition-all group">
              <div className={`w-14 h-14 bg-${s.c}-50 text-${s.c}-600 rounded-2xl flex items-center justify-center text-2xl mb-6 group-hover:scale-110 transition-transform`}>{s.i}</div>
        ...
        <div className="bg-white p-12 rounded-[3rem] border border-gray-50 h-80 flex items-center justify-center text-gray-300 font-black uppercase tracking-widest text-xs italic">
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] p-6 text-center">
            <div className="max-w-md w-full bg-white p-12 rounded-[3rem] shadow-2xl border border-gray-100">
                <h2 className="text-3xl font-black mb-4 text-gray-900 tracking-tight leading-none">Crea tu equipo</h2>
                <p className="mb-10 text-gray-400 text-sm font-medium">Configura tu espacio de trabajo bilingüe en segundos.</p>
                <form onSubmit={handleCreate} className="space-y-6">
                    ...
                    <Button type="submit" variant="dark" className="w-full py-5 text-xs" disabled={loading}>{loading ? 'CREANDO...' : 'CONFIRMAR Y EMPEZAR'}</Button>
                </form>
            </div>
        </div>
    );
      ...
      <HashRouter>
        <Routes>
          ...
          <Route path="/t/:slug" element={<TenantLayout />}>
            ...
            <Route path="*" element={<div className="bg-white p-20 rounded-3xl border text-center text-gray-300 font-bold uppercase tracking-widest text-xs">Módulo en Desarrollo</div>} />
          </Route>
