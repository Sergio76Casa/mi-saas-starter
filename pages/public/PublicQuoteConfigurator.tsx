...
export const PublicQuoteConfigurator: React.FC<PublicQuoteConfiguratorProps> = ({
  ...
  const currentTotal = useMemo(() => {
    const variantPrice = product.pricing?.[selectedVariantIdx]?.price || 0;
    const kitPrice = product.installation_kits?.[selectedKitIdx]?.price || 0;
    const extrasTotal = (product.extras || []).reduce((acc, curr, idx) => {
      const qty = extraQuantities[idx] || 0;
      return acc + (qty * (curr.unit_price || 0));
    }, 0);
    return variantPrice + kitPrice + extrasTotal;
  }, [product, selectedVariantIdx, selectedKitIdx, extraQuantities]);
          ...
          <div className="bg-slate-900 rounded-[2.5rem] p-8 md:p-10 shadow-2xl relative overflow-hidden text-left">
            ...
            <div className="mb-10 text-center flex flex-col items-center">
              <div className="w-full h-40 bg-white/5 rounded-3xl p-6 flex items-center justify-center mb-6">
                {product.image_url ? <img src={product.image_url} className="w-full h-full object-contain" alt="" /> : <div className="text-slate-700">NO IMAGE</div>}
              </div>
                ...
                <div className="text-right space-y-1">
                  {Object.keys(extraQuantities).filter(k => extraQuantities[parseInt(k)] > 0).length > 0 ? (
                    Object.entries(extraQuantities).map(([idx, qty]) => {
                      ...
                      const extra = product.extras?.[parseInt(idx)];
                      return (
            ...
            <div className="space-y-4">
              <button 
                ...
                onClick={() => product.pdf_url && window.open(product.pdf_url, '_blank')}
                className="w-full py-4 bg-white/5 hover:bg-white/10 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all border border-white/5 flex items-center justify-center gap-3 disabled:opacity-30 disabled:cursor-not-allowed"
              >
