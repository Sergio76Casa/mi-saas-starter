...
export const QuoteAcceptancePage = () => {
    ...
    const fetchQuote = async () => {
      try {
        ...
        setClientData(prev => ({
          ...
          dni: q.client_dni || '',
          population: q.client_population || ''
        }));
  ...
  const syncCustomer = async () => {
    if (!quote || !clientData.email) return null;

    const fullName = `${clientData.name} ${clientData.surname}`.trim();

    // 1. Buscar si el cliente ya existe en la tabla customers
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          client_dni: clientData.dni,
          client_population: clientData.population,
          ...
          <div className="lg:col-span-7 space-y-12">
            <section className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1">DNI/NIF</label>
                  <input value={clientData.dni} onChange={e => setClientData({ ...clientData, dni: e.target.value })} className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Población</label>
                  <input value={clientData.population} onChange={e => setClientData({ ...clientData, population: e.target.value })} className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
              </div>
