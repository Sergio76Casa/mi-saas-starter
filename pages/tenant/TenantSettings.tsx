...
import { Tenant, Branch, BusinessHours } from '../../types';
  ...
  return (
    <div className="max-w-4xl animate-in fade-in duration-500 mx-auto md:mx-0 pb-20 text-left">
      ...
      <div className="space-y-10">
        ...
        {/* REDES SOCIALES */}
        <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm space-y-8">
          <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] italic text-left">Links de Redes Sociales</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Input disabled={!isAdmin} label="Instagram (URL)" value={socials.social_instagram} onChange={(e: any) => setSocials({ ...socials, social_instagram: e.target.value })} />
            <Input disabled={!isAdmin} label="Facebook (URL)" value={socials.social_facebook} onChange={(e: any) => setSocials({ ...socials, social_facebook: e.target.value })} />
            <Input disabled={!isAdmin} label="TikTok (URL)" value={socials.social_tiktok} onChange={(e: any) => setSocials({ ...socials, social_tiktok: e.target.value })} />
            <Input disabled={!isAdmin} label="WhatsApp (Teléfono)" value={socials.social_whatsapp} onChange={(e: any) => setSocials({ ...socials, social_whatsapp: e.target.value })} />
            <Input disabled={!isAdmin} label="LinkedIn (URL)" value={socials.social_linkedin} onChange={(e: any) => setSocials({ ...socials, social_linkedin: e.target.value })} />
            <Input disabled={!isAdmin} label="X / Twitter (URL)" value={socials.social_x} onChange={(e: any) => setSocials({ ...socials, social_x: e.target.value })} />
            <Input disabled={!isAdmin} label="YouTube (URL)" value={socials.social_youtube} onChange={(e: any) => setSocials({ ...socials, social_youtube: e.target.value })} />
            <Input disabled={!isAdmin} label="Telegram (URL)" value={socials.social_telegram} onChange={(e: any) => setSocials({ ...socials, social_telegram: e.target.value })} />
          </div>
        </div>
              ...
              <div key={i} className="p-6 bg-slate-50 rounded-3xl border-slate-100 border relative text-left">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2 mt-4">
                    <BusinessHoursEditor
                      disabled={!isAdmin}
                      value={b.business_hours}
                      onChange={(val) => { const c = [...branches]; c[i].business_hours = val; setBranches(c); }}
                    />
                  </div>
                </div>
              </div>
